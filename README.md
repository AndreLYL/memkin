...
Memkin 是一个**面向中国职场、本地优先的个人记忆系统**。中国职场的工作发生在飞书、钉钉、企业微信里 —— 我们把这些工具里的私聊、群聊、邮件、会议、文档,连同你与 AI Agent 的会话,一起提取成结构化信号（实体、决策、任务、发现、知识、关系），汇入你自己机器上一个统一、可搜索的知识图谱，再通过 **MCP** 把这份记忆喂回给任何 Agent。
...
Memkin 是标准 MCP stdio 服务器，可接入任何 MCP 客户端：
...
| | Memkin | 纯 RAG / 向量检索 | 笔记工具（Obsidian / Notion） | GBrain | OpenHuman |
|---|:---:|:---:|:---:|:---:|:---:|
| 本地优先 & 私密 | ✅ | 视情况 | 视情况 | ✅ | ✅ |
| 开源 | ✅ | 不一定 | 部分 | 部分 | ✅ |
...
> 纯 RAG 只有向量、没有实体和关系，回答缺乏上下文；笔记工具强大但依赖手动维护。Memkin 保持本地、Agent 原生 —— 并把飞书工作作为一等数据源。
...
> **对外暴露与鉴权**：服务默认只绑定 `127.0.0.1`（仅本机）。要在局域网暴露，用 `memkin serve --host 0.0.0.0`（或在 `memkin.yaml` 里设 `server.host`）——这会**强制要求配置鉴权令牌**，否则拒绝启动。令牌来自 `server.auth_token`（config）或 `MEMKIN_AUTH_TOKEN`（env）；配置后，所有 API 请求都需带 `Authorization: Bearer <token>` 请求头。
>
> ```yaml
> # memkin.yaml
> server:
...
`memkin init` 启动一个**交互式配置中心**（基于 React + ink 的全屏 TUI），让你零手写地生成 / 编辑 `memkin.yaml`：
...
- 🔒 **密钥脱敏**：API key 始终掩码显示
- 🧭 **自动检测**：识别已有数据源（Claude Code、Codex、Hermes）并注册 `memkin` 命令

**运行模式：**

| 命令 / 环境 | 行为 |
|---|---|
| `memkin init`（TTY 终端下） | 全屏 TUI 配置中心 |
...
> `memkin config init` 与 `memkin init` 等价。飞书等少数高级配置目前需直接编辑 `memkin.yaml`（见下方飞书章节）。
...
本地需要先完成 lark-cli 的飞书用户态登录，并在 `memkin.yaml` 打开 `message_search`：
...
Memkin 可以把飞书文档采集为可升级的"摘要卡片"：先建立轻量的 pointer 卡，被触发后再升级为完整摘要卡。
...
memkin docs retry <doc_token>
memkin docs retry --all-failed
```
...
- 运行状态：当前仓库的 `.memkin/cursors.yaml` 和 `.memkin/dedup.jsonl`，分别保存源 cursor 和消息去重 hash。

正常增量运行不要手动删这些文件。需要删除某个源的过期提取结果并重跑时，用内置命令，它会先备份数据库和 `.memkin`：
...
> 缺少 `memkin.yaml` 时 `serve` 会提示先用 `memkin start` 一步配置 + 启动，或 `memkin init --web` 先完成配置。

### 接入你的 Agent（MCP）

**一键接入（推荐）**：`memkin install` 会自动把 MCP 配置 + 一份极简「记忆指令」写进你的 AI 客户端（**默认全局**，对所有项目生效），支持 **Claude Code · Claude Desktop · Cursor · Codex · Windsurf**：
...
**让 Agent 自己接入**：对能读取外链的 Agent，直接说「按 [`MEMKIN_FOR_AGENTS.md`](MEMKIN_FOR_AGENTS.md) 把我接入 Memkin」，它会自跑上面的命令完成接入并自检。**OpenClaw / Hermes** 用 `memkin install --agent hermes`（写 `config.yaml` 的 `mcp_servers` + 铺 `memkin` skill，会话里 `/reload-mcp` 生效）；也可 `memkin skill scaffold --dir ~/.hermes/skills` 单独铺 skill。

Memkin 提供两种 MCP 接入方式，按场景选：

- **stdio（`--mcp`）** —— 本地直连，Agent 把 `memkin` 作为子进程拉起，零网络配置，单机单客户端首选。
- **Streamable HTTP（`--mcp-http`）** —— 走 HTTP（默认 `3928`），适合远程接入或多个客户端共享同一份记忆。

让任何 MCP 客户端指向 Memkin，即可读写你的记忆。以 Claude Code 为例（stdio 本地直连）：
...
Memkin 是 **5 层纵向数据流 + 3 个横切关注点**。数据自上而下流动:数据源被采集、提取成信号、存入本地记忆,再由底层接口对外读写;**人物身份**与**记忆巩固**、**调度**则横切贯穿其间。
...
| 层 | 职责 |
|----|------|
| **① 配置与上手层** | TUI 配置中心(React + ink)、Web UI 配置、`memkin.yaml` 手编;自动检测运行时 / API key / 数据源,硬件评估推荐 Embedding,实时连接测试 |
...
| `memkin identity` | 人物身份管理：别名、合并、重命名 |
