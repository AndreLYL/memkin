...
**🤖 让 Agent 懂你（MCP）**
把 Memoark 作为任何 MCP Agent 的记忆层 —— Claude Code、Cursor、Windsurf。默认暴露 **10 个首选记忆工具**，支持统一来源过滤、结构化错误、Resources、Prompts 和可追溯 provenance。Agent 既是记忆的生产者，也是消费者。
...
| 实体 + 关系知识图谱 | ✅ | ❌ | 手动 | ✅ | 部分 |
| 结构化信号提取（不只是分块） | ✅ | ❌ | ❌ | ✅ | ✅ |
| 精简的 MCP 工具面（默认 10 个首选工具，而非 40+） | ✅ | 不适用 | 不适用 | ❌（40+） | 不一定 |
...
cd memoark
bun install              # 安装主项目依赖
cd web && bun install && cd ..   # 安装 Web UI 依赖
...
# MCP Streamable HTTP（默认仍建议本地使用；远程前必须配置安全项）
...
| 工具 | 类型 | 用途 |
|---|---|---|
| `query` | read | 语义回忆，适合模糊、概念性、跨来源问题 |
| `search` | read | 精确关键词搜索 |
| `get_page_context` | read | 一次读取页面、标签、关系、时间线和 chunk |
| `timeline_feed` | read | 全局时间线回顾，支持来源和时间过滤 |
| `explore_graph` | read | 探索页面附近的知识图谱关系 |
| `put_page` | write | 幂等写入或更新页面 |
| `add_timeline_entry` | write | 追加页面时间线事件 |
| `manage_links` | write | 添加或移除页面关系 |
| `manage_tags` | write | 添加或移除页面标签 |
| `get_health` | diagnostic | 查看 MCP 版本、能力和健康状态 |

读取工具共享统一过滤参数：`platform`、`source_type`、`channel`、`channel_name`、`participant`、`from`、`to`、`type`、`exclude_types`、`limit`。默认工具列表不会出现 `query_wechat`、`search_feishu` 这类按来源命名的普通检索工具；旧 CRUD/debug 工具只有在 `mcp.expose_legacy_tools: true` 时才暴露。

MCP Resources：

| Resource | 内容 |
|---|---|
| `memoark://health` | 健康、版本和能力元数据 |
| `memoark://pages` | 有界页面索引 |
| `memoark://pages/{slug}` | 页面内容，slug 需要 URL 编码 |
| `memoark://pages/{slug}/context` | 页面综合上下文 |
| `memoark://pages/{slug}/timeline` | 页面时间线 |

MCP Prompts：`recall`、`weekly-digest`、`who-is`、`decision-log`、`handoff`。这些 Prompt 只引用首选工具和统一 filter，不硬编码具体数据源工具。

远程 MCP 使用 `streamable_http` 时默认建议只绑定本机地址，并配置 `allowed_origins`、`allowed_hosts` 和 `auth_token_env`。公网绑定必须提供 token 环境变量；需要只读远程能力时设置 `mcp.http.read_only: true`，写工具会从 MCP 工具列表移除。
...
memoark serve --mcp-http   # MCP Streamable HTTP 传输
```
...
- [x] Hono REST API
- [x] MCP 服务器（默认 10 个首选工具，legacy/debug 工具可配置开启）
