...
- `Scheduler`（`src/daemon/scheduler.ts`）按 interval 定时触发，状态存 `scheduler-state.json`
- `runPipeline()`（`src/core/pipeline.ts`）同步执行，返回 `PipelineResult`
- 飞书子源通过 `lookback_days` 控制抓取范围，无 `until` 参数
- Spec 5 的 `/config` 设置页不含 `SchedulerConfig` 配置

### 历史回溯时间范围简化

**只支持 `since_ms`（左边界），右边界始终为今天。** 原因：添加 `until` 参数需修改所有飞书子 Collector 接口，超出本 spec 范围。`since_ms` → `lookback_days = ceil((now - since_ms) / 86400000)` 直接复用现有 Collector 配置。已索引数据通过 Pipeline 内置去重（cursor / dedup）跳过，不会重复写入。
...
interface SourceProgress {
  source: string;   // "dm" | "messages" | "mail" | "docs" | "tasks" | "calendar"
  processed: number;
  status: "pending" | "running" | "done" | "error" | "skipped";
  message?: string; // 最新动态行，如 "正在处理：oc_abc123 群 2024-03-15"
}
...
interface BackfillOpts {
  since_ms: number;
  source_types: Array<"dm" | "messages" | "mail" | "docs" | "tasks" | "calendar">;
  configPath: string;
}
```
...
`runPipeline()` 调用方式：读取磁盘 config，按 source_type 临时覆盖 `feishu.sources.<type>.lookback_days` = `ceil((now - since_ms) / 86400000)`，其余配置不变。

### 覆盖密度（`GET /api/backfill/coverage`）

从 `timeline_entries` 按 `source = 'feishu'` 过滤飞书条目，按 `date`（事件发生日期，TEXT ISO-8601）以 7 天为一桶聚合，返回最近 104 周（2 年）。`timeline_entries.source` 存储 `SourceRef.platform`，即 `"feishu"`（非 `"feishu/xxx"`）。无需新表或迁移。
...
| Method | Path | 说明 |
|--------|------|------|
| `POST` | `/api/backfill/start` | Body: `{ since_ms: number; source_types: string[] }` → 启动任务；已有任务返回 409 |
| `POST` | `/api/backfill/cancel` | 取消当前任务，幂等 |
| `GET` | `/api/backfill/status` | 返回 `BackfillStatus` |
| `GET` | `/api/backfill/coverage` | 返回时间桶密度，需 `StoreContext`（`PageStore`） |
...
- 默认抓取间隔（`scheduler.defaults.interval_secs`，秒）
- 飞书各子源独立 interval（`scheduler.sources.feishu_<type>.interval_secs`，留空 = 用默认值）
...
**idle 状态：**
```
回溯起始：2024-01-15（距今约 5 个月）

数据源：[✓] 群聊  [✓] DM  [✓] 邮件  [ ] 云文档  [ ] 任务  [ ] 日历
...
↳ 正在处理：oc_abc123 群 2024-03-15
```

进度条宽度：`processed / maxProcessed * 100%`（各源互相比较最大值，不显示百分比）

**done 状态：**
```
✓ 回溯完成（3m 42s）

共处理 598 条消息 → 生成 87 个 block → 写入 52 个页面
...
- 回溯进度百分比（当前 Collector 不暴露 total 估算）
- 覆盖热力图按子源分层显示
- `until_ms` 支持（右边界限制）
