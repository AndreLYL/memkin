...
> **实际输出的二级标题以 §5.1 systemPrompt 固定的 7 个为准**（`## 今日概览 / ## 今日完成 / ## 推进中 / ## 我的待办 / ## 待回复与被@ / ## 人脉动态 / ## 明日提醒`，`parseSections` 依赖完全一致）。下表是各段的语义说明：

1. **今日概览**（一句话 + 关键数字）
2. **今日完成**（达成的决策 + 完成的任务）
3. **推进中**（按项目实体分组的话题）
4. **我的待办**（新增 / 完成 / 逾期）
5. **待回复与被@**（私聊群聊里待我回复 / 被@未回）
6. **人脉动态**（今天和谁有重要互动、关系进展）← 原创，呼应主线
7. **明日提醒**（日历会议 + 到期任务）
