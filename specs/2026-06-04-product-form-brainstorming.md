...
> **注意**：本节是早期讨论记录。"原始信号保留 30天""标记永久保留"等内容已被 Spec 2 §八明确修正（pipeline 不持久化原始消息，30天 TTL 无删除对象）。**以 `spec2` 为准**，不以本节为准。
...
> **注意**：本节是早期讨论记录，存在过时内容（8种/procedures独立/concept sub_type等）。**以 `spec1` 为准**，不以本节为准。
