...
- **影响**：现有对排序顺序有硬断言的测试可能变红 → 本 spec **负责审查并更新这些测试**（明确允许排序结果变化），或保留开关由配置控制。
- 提供配置 `search.pool_by_page`（默认 true），可回退。
...
新增 `src/store/wikilink.ts`：`put_page` 写入后，扫描 `compiled_truth` 中的 **wikilink 语法 `[[slug]]` 与 typed-link `[[rel:slug]]`**，确定性建边：
...
- **校验**：`to` slug 不存在时跳过（或建占位指针），不报错中断写入。
- 作为 **LLM 抽边的便宜兜底/补充**，二者并存。

`type` 仅接受 `LinkType` union（`core/types.ts:109`）内的值，未知 rel 归 `custom`。
...
- **起步：规则式**（零 LLM）——CJK 分词归一、同义词/缩写扩展（可配置词表）、去停用词。
