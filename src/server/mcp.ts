...
export function createMcpToolHandlers(stores: StoreContext) {
  return {
    query: ({ query, limit }: { query: string; limit?: number }) => stores.search.query(query, { limit }),
    search: ({ query, limit }: { query: string; limit?: number }) => stores.search.search(query, { limit }),
    ...
    add_link: async ({ from, to, type, context }: { from: string; to: string; type?: string; context?: string }) => {
    ...
    traverse_graph: ({ slug, depth, direction }: { slug: string; depth?: number; direction?: "in" | "out" | "both" }) =>
    ...
    add_timeline_entry: async (entry: { slug: string; date: string; summary: string; detail?: string; source?: string }) => {
    ...
    get_health: async () => {
      ...
      return { status: "ok", pages: Number(pages.rows[0].c), chunks: Number(chunks.rows[0].c) };
    },
  };
}
...
export function createMcpServer(stores: StoreContext): McpServer {
  ...
  server.tool("add_link", { from: z.string(), to: z.string(), type: z.string().optional(), context: z.string().optional() }, async (args) => text(await tools.add_link(args)));
  ...
  server.tool("traverse_graph", { slug: z.string(), depth: z.number().optional(), direction: z.enum(["in", "out", "both"]).optional() }, async (args) => text(await tools.traverse_graph(args)));
  ...
  server.tool("add_timeline_entry", { slug: z.string(), date: z.string(), summary: z.string(), detail: z.string().optional(), source: z.string().optional() }, async (args) => text(await tools.add_timeline_entry(args)));
