...
// demand as a skill. Single source of truth for `memoark skill scaffold` and
// the Hermes adapter.
export const MEMOARK_SKILL = `---
name: memoark
...
# Memoark — your personal memory

Memoark is the user's **local-first** personal memory and the source of truth about their
people, projects, decisions, tasks, and history. It is exposed over MCP.

## When to use it
Consult Memoark **before answering** whenever a request concerns:
...
Do **not** consult Memoark for generic questions unrelated to the user's world
...
For **project-status** questions, combine Memoark (decisions / tasks / timeline) **with**
...
## Examples
- *"Where is the memoark project at?"* → \`get_session_context\` + \`query("memoark 进展")\`, then merge with the repo's git/code reality.
- *"What did Xu Lizi talk to me about last week?"* → \`recall(entity="entities/xu-lizi")\` or \`query("许力子 上周")\`.
`;

/** Write the memoark skill into `<dir>/memoark/SKILL.md`. Idempotent. Returns the path. */
export function scaffoldSkill(dir: string): string {
  const skillDir = join(dir, "memoark");
  mkdirSync(skillDir, { recursive: true });
  const path = join(skillDir, "SKILL.md");
  writeFileSync(path, MEMOARK_SKILL);
  return path;
}
