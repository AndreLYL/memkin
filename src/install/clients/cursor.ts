...
import { DIRECTIVE_L1, MEMOARK_BLOCK_END, MEMOARK_BLOCK_START } from "../directive.js";
import { type ClientAdapter, mcpEntry, type PlanCtx } from "../types.js";

// Cursor rules use `.mdc` with required frontmatter, so memoark owns the whole
// file (managed-file) rather than appending a marker block.
const L1_BODY = DIRECTIVE_L1.replace(MEMOARK_BLOCK_START, "").replace(MEMOARK_BLOCK_END, "").trim();
const CURSOR_MDC = `---\nalwaysApply: true\n---\n\n${L1_BODY}\n`;
...
export const cursor: ClientAdapter = {
  ...
  plan(ctx: PlanCtx) {
    ...
    const rulesPath = join(base, ".cursor", "rules", "memoark.mdc");
    return [
      {
        path: mcpPath,
        kind: "json-mcp",
