// Single source of truth for Memkin's agent-facing memory directives.
//
...
export const MEMKIN_BLOCK_START = "<!-- memkin:start -->";
export const MEMKIN_BLOCK_END = "<!-- memkin:end -->";

/** L1: minimal directive block for rules files (CLAUDE.md / AGENTS.md / …). */
export const DIRECTIVE_L1 = `${MEMKIN_BLOCK_START}
## Memkin — 你的个人记忆库
你接入了 Memkin：用户关于「人 / 项目 / 决策 / 任务 / 过往」的本地持久记忆，是这些事的**事实来源**。
- **何时查**：问题涉及具体的人、项目进展、过去的决定、待办，或带「上周 / 之前 / 我们当时…」时——先查 Memkin，别只凭代码或猜测。
...
export const DIRECTIVE_L2 = `Memkin is the user's local-first personal memory — the source of truth about the user's people, projects, decisions, tasks, and history.

Brain-first, cheap-first. When a request concerns a specific person, a project's status/history, a past decision, a todo, or contains "last week / earlier / what did we…", consult Memkin BEFORE answering from code or assumptions:
...
Project-status questions: combine Memkin (decisions/tasks/timeline) WITH repo reality (git/code); do not answer from code alone.
Do NOT consult Memkin for generic questions unrelated to the user's world (pure syntax, general algorithms).
