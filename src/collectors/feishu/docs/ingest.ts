...
async function fetchDocMeta(
  ...
  const res = await client.request<{
    code: number;
    data?: { metas?: Array<{ title?: string; url?: string; owner_id?: string; create_time?: string; latest_modify_time?: string }> };
...
export async function ingestFeishuDoc(deps: IngestDeps, input: IngestInput): Promise<IngestOutput> {
  ...
  if (parsed.kind === "reject") {
    if (parsed.code === "UNSUPPORTED_DOC_TYPE") {
      return { ok: false, error: { code: "UNSUPPORTED_DOC_TYPE", type: "non_docx", message: parsed.message } };
    }
  ...
  try {
    if (parsed.kind === "wiki_node") {
      ...
      if (resolved.obj_type !== "docx") {
        return { ok: false, error: { code: "UNSUPPORTED_DOC_TYPE", type: resolved.obj_type, message: "Wiki node is not a docx" } };
      }
  ...
  const candidate: DocCandidate = {
    ...
    created_at: meta.created === "0" ? now : new Date(Number.parseInt(meta.created, 10) * 1000).toISOString(),
    modified_at: meta.modified === "0" ? now : new Date(Number.parseInt(meta.modified, 10) * 1000).toISOString(),
  ...
  const card = await builder.build(candidate, { userNote: input.note, tags: input.tags, force: true });
  ...
  if (card.extract_level === "pointer") {
    ...
    return {
      ok: false,
      error: { code: "LLM_FAILED", doc_token: docToken, saved_as: "pointer", original_error: card.extract_error ?? card.extract_skipped ?? "unknown" },
    };
  }

  const finalCard = input.note ? mergeUserNoteIntoCard(card, input.note) : card;
  await writeCard(deps.stores, finalCard);
  return { ok: true, doc_token: docToken, extract_level: "full", was_existing: existing !== null, card: finalCard };
}
