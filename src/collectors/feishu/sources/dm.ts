...
export class DMSource implements FeishuSource {
  readonly name = "dm";
  ...
  constructor(
    ...
    this.selfOpenId = opts.selfOpenId;
  }
  ...
  async *fetch(
    ...
    for (let i = 0; i < this.chatIds.length; i++) {
      ...
      if (settled.status === "rejected") {
        console.error(`[DMSource] Failed to fetch chat ${chatId}:`, settled.reason);
        continue;
      }
  ...
  private mapMessage(msg: FeishuMessage, chatId: string): RawMessage {
    ...
    const direction = msg.sender.id === this.selfOpenId ? "sent" : "received";
