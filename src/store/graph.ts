...
export class GraphStore {
  ...
  async traverse(slug: string, opts?: { depth?: number; direction?: "in" | "out" | "both" }): Promise<GraphNode[]> {
      ...
      for (const current of frontier) {
        ...
        for (const n of neighbors) {
          if (!visited.has(n)) {
            visited.add(n);
            nextFrontier.push(n);
            const page = await this.pg.query("SELECT slug, title, type FROM pages WHERE slug = $1", [n]);
