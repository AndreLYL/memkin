...
**30 built-in MCP tools** let any agent both query and write back to your memory. Full Feishu capture (7 sources) turns your real work — requirements, proposals, team decisions — into a first-class data source, something neither pure RAG nor note apps can do.
...
Use Memoark as the memory layer for any MCP agent — Claude Code, Cursor, Claude Desktop, Windsurf. **30 built-in tools** let your agent query your history, read entity pages, and write new knowledge back. Agents are both producers and consumers of your memory.
...
| **⑤ Interfaces & Consumption** | CLI, MCP Server (30 tools — agent read / write / maintain), REST API (Hono), Web UI (search / view / graph / timeline, **read-only today**), Obsidian bidirectional sync |
...
Memoark's MCP server exposes **30 tools** spanning retrieval, synthesis, page CRUD, graph, tags, timeline, identity, and Feishu doc ingestion. Prefer the high-level tools first:

| Category | Tools |
|----------|-------|
| **Retrieval (high-level)** | `query`, `get_session_context`, `get_entity_profile`, `list_signals_by_entity` |
| **Synthesis** | `synthesize`, `recall` (cited, gap-aware composed answers with inline `[n]`), `prep_for_person` (person communication profile → goal-conditioned communication strategy; passively inferred, no questionnaire, local-first, ethics-guardrailed), `daily_report(date?)` (cross-channel daily report in 7 fixed sections: overview / done / in-progress / my to-dos / awaiting-reply & @mentions / people updates / tomorrow reminders) |
| **Search** | `search` |
...
- [x] Hono REST API
- [x] MCP Server with 30 stdio tools
...
- [x] **Cross-channel daily report**: `daily_report(date?)` — synthesizes the day's mail / IM / calendar / doc signals into a fixed 7-section report (highlighting "my to-dos" via the `entities/me` self-identity), `[n]`-cited + gap-aware, cached per day at `reports/daily/<date>`
- [x] **Meeting decision / action-item extraction**: Feishu doc full cards extract `decisions[]` and owner/due-bearing `action_items[]`, persisting each action item as a `type=task` signal (anchored to its owner, and to `entities/me` when the owner is me)
