...
describe("AlertWriter", () => {
  ...
  it("deletes alert page when no sources have alerts", async () => {
    ...
    await writer.update([
      {
        source_id: "feishu",
        state: { last_run_at: Date.now(), last_result: "failed", last_error: "err", consecutive_failures: 3, consecutive_partials: 0 },
      },
    ]);
  ...
  it("includes multiple sources in one alert page", async () => {
    const writer = new AlertWriter(pageStore);
    await writer.update([
      { source_id: "feishu", state: { last_run_at: Date.now(), last_result: "failed", last_error: "timeout", consecutive_failures: 5, consecutive_partials: 0 } },
      { source_id: "codex", state: { last_run_at: Date.now(), last_result: "partial", last_error: null, consecutive_failures: 0, consecutive_partials: 7 } },
    ]);
