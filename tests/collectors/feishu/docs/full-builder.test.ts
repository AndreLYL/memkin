...
const cand: DocCandidate = {
  doc_token: "t", doc_type: "docx", title: "T", url: "u", owner_id: "o", last_editor_id: "e",
...
function clientWithBlocks(blocks: unknown[], rawLen = 500) {
  ...
  const allBlocks = [{ block_type: 2, text: { elements: [{ text_run: { content: pad } }] } }, ...blocks];
  return {
...
describe("FullCardBuilder", () => {
  test("builds a full card from blocks + LLM", async () => {
    ...
    const builder = new FullCardBuilder(clientWithBlocks([{ block_type: 3, heading1: { elements: [{ text_run: { content: "Goals" } }] } }]) as never, provider, "mock-model", () => "2026-06-14T00:00:00Z");
  ...
  test("degrades to pointer with extract_error on unparseable LLM output", async () => {
    ...
    const builder = new FullCardBuilder(clientWithBlocks([]) as never, provider, "mock-model", () => "2026-06-14T00:00:00Z");
    const result = await builder.build(cand);
    expect(result.extract_level).toBe("pointer");
