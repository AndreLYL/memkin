...
import { batchSizeForRun, UpgradeQueue } from "../../../../src/collectors/feishu/docs/upgrade-queue";
...
describe("batchSizeForRun", () => {
  test("uses bootstrap size during bootstrap window", () => {
    expect(batchSizeForRun(0, { batch_size: 20, bootstrap_batch_size: 50, bootstrap_runs: 5, max_pending: 5000 })).toBe(50);
    expect(batchSizeForRun(4, { batch_size: 20, bootstrap_batch_size: 50, bootstrap_runs: 5, max_pending: 5000 })).toBe(50);
  });

  test("uses steady-state size after bootstrap", () => {
    expect(batchSizeForRun(5, { batch_size: 20, bootstrap_batch_size: 50, bootstrap_runs: 5, max_pending: 5000 })).toBe(20);
  });
});
