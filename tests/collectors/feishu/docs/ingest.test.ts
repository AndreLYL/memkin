  ...
  const client = {
    ...
    async *paginate(path: string) {
      if (path.includes("/blocks"))
        yield {
          items: [
            { block_type: 2, text: { elements: [{ text_run: { content: "x".repeat(500) } }] } },
          ],
          has_more: false,
        };
...
describe("ingestFeishuDoc", () => {
  test("rejects unsupported doc type", async () => {
    const { deps: d } = deps();
    const out = await ingestFeishuDoc(d as never, {
      url_or_token: "https://feishu.cn/sheets/shtcn123456789012345",
    });
  ...
  test("resolves a wiki node URL to its docx obj_token", async () => {
    const { deps: d } = deps();
    const out = await ingestFeishuDoc(d as never, {
      url_or_token: "https://feishu.cn/wiki/Node1234567890abcdef",
    });
    expect(out.ok).toBe(true);
