...
describe("MCP server", () => {
  ...
  beforeEach(async () => {
    ...
    stores = {
      ...
      search: new SearchEngine(db.pg, { embedText: vi.fn().mockResolvedValue(Array(1536).fill(0.1)) }),
  ...
  it("tool handlers can put and get pages", async () => {
    ...
    await tools.put_page({ slug: "entities/alice", content: "---\ntitle: Alice\ntype: person\n---\nAlice." });
  ...
  it("tool handlers expose query, graph, tags, timeline, chunks, and health", async () => {
    const tools = createMcpToolHandlers(stores);
    await tools.put_page({ slug: "entities/alice", content: "---\ntitle: Alice\ntype: person\n---\nAlice." });
    await tools.put_page({ slug: "projects/memoark", content: "---\ntitle: Memoark\ntype: project\n---\nMemoark." });
    ...
    await tools.add_timeline_entry({ slug: "entities/alice", date: "2026-05-26", summary: "Started Memoark" });
    ...
    expect(await tools.traverse_graph({ slug: "entities/alice", depth: 1, direction: "out" })).toHaveLength(1);
