...
describe("REST API", () => {
  ...
  it("supports graph, tags, timeline, chunks, and embed routes", async () => {
    ...
    expect((await app.request("/tags?slug=entities/alice", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ tag: "person" }) })).status).toBe(200);
    expect((await app.request("/timeline?slug=entities/alice", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ date: "2026-05-26", summary: "Started Memoark" }) })).status).toBe(200);
    expect((await app.request("/links", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ from: "entities/alice", to: "projects/memoark", type: "works_on" }) })).status).toBe(200);
    ...
    expect((await app.request("/graph/traverse?slug=entities/alice&depth=1&direction=out")).status).toBe(200);
