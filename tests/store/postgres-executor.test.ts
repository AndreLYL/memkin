...
d("PostgresExecutor", () => {
  it("query {rows}, multi-statement exec, transaction commit + rollback", async () => {
    ...
    await ex.exec(
      "CREATE TABLE t (id int, e vector(3)); CREATE INDEX ix ON t USING hnsw (e vector_cosine_ops)",
    );
    ...
    await expect(
      ex.transaction(async (tx) => {
        await tx.query("INSERT INTO t VALUES (2,'[0,0,0]')");
  ...
  it("bootstrap runs fn under advisory lock", async () => {
    ...
    let ran = false;
    await ex.bootstrap(async (conn) => {
      await conn.exec("CREATE TABLE b (id int)");
      ran = true;
    });
