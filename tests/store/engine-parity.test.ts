...
describe.each(cases)("PageStore + memoark_meta parity on $name", ({ makeConfig }) => {
  it("putPage/getPage roundtrip and memoark_meta exists", async () => {
    ...
    // memoark_meta table must exist (created by schema bootstrap)
    const meta = await db.executor.query<{ c: number }>("SELECT count(*)::int c FROM memoark_meta");
